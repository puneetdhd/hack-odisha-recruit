# Fixed voice_functions.py - Real-time Voice Verification System for Interview Platform
import numpy as np
import librosa
...
class RealTimeVoiceProcessor:
    ...
    def process_audio_bytes(self, audio_bytes: bytes, audio_format: str = 'wav') -> Optional[np.ndarray]:
        ...
        try:
            ...
            # Try to identify actual format from magic bytes
            ...
            # Method 1: Try with temporary file (most reliable)
            try:
            ...
            # Method 2: Try direct numpy conversion for simple formats
            try:
            ...
            # Method 3: Try with BytesIO and librosa
            try:
    ...
    def _process_raw_audio(self, audio_bytes: bytes) -> Optional[np.ndarray]:
        ...
        try:
            # Try to parse as 16-bit WAV
            if len(audio_bytes) < 44:  # WAV header is 44 bytes minimum
                return None
            
            # Skip WAV header and read raw PCM data
            pcm_data = audio_bytes[44:]  # Skip standard WAV header
            
            # Convert to numpy array (assuming 16-bit little-endian)
            audio = np.frombuffer(pcm_data, dtype=np.int16).astype(np.float32)
            
            # Normalize to [-1, 1]
            audio = audio / 32768.0
            
            # Convert to mono if stereo (assume interleaved)
            if len(audio) % 2 == 0:
                # Try stereo to mono conversion
                try:
...
class RealTimeFeatureExtractor:
    ...
    def extract_features(self, audio: np.ndarray) -> Optional[np.ndarray]:
        ...
        try:
            ...
            # MFCC features (most important for voice identification)
    ...
    def _extract_mfcc_stats(self, audio: np.ndarray) -> Optional[List[float]]:
        ...
        try:
            ...
            features = []
            # Statistical measures for MFCC and delta-MFCC
